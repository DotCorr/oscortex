...
fn parse_dart_snapshot_elf(aot_va: u64, size: usize) -> Option<DartSnapshotPointers> {
    ...
    for i in 0..e_phnum {
        ...
        if off + 56 > buf.len() { return None; }
        let p_type = read_u32(buf, off)?;
        if p_type != PT_LOAD { continue; }
        if nloads >= loads.len() { break; }
    ...
    for i in 0..e_shnum {
        ...
        match sh_type {
            SHT_SYMTAB => {
                sym_off = sh_offset; sym_size = sh_size;
                sym_entsz = sh_entsize; sym_link = sh_link;
            }
            SHT_DYNSYM => {
                dyn_off = sh_offset; dyn_size = sh_size;
                dyn_entsz = sh_entsize; dyn_link = sh_link;
            }
            _ => {}
        }
    }
...
fn debug_hex(label: &[u8], val: u64) {
    let mut line = [0u8; 80];
    let mut n = 0;
    for &b in label { if n < line.len() { line[n] = b; n += 1; } }
    n = write_hex64(&mut line, n, b"0x", val);
    if n < line.len() { line[n] = b'\n'; n += 1; }
    write(&line[..n]);
}
